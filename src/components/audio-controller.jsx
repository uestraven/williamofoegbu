...
const DownloadButton = ({ path, text }) => {
    ...
    return (
        <DownloadLink href={path} download onClick={handleDownload}>
            {text && text}
            {' '}
            <SVGWrapper aria-hidden="true" focusable="false" data-prefix="fas" data-icon="download" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="button svg-inline--fa fa-download fa-w-16 fa-fw fa-lg" style={{ height: '12px' }}>
...
const AudioController = ({ audio, title, show, animate }) => {
    ...
    return (
        ...
        >
            ...
            <DownloadRow className={showController ? 'openaudio' : null}>
                <DownloadButton path={audio} text="Download" />
            </DownloadRow>
        </AudioControllerWrapper>
    );
};
